    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Insert()
        {
            ...
            for (var i = 0; i < obj1.Count; i++)
            {
                ...
                string sql = $"begin insert into LBS_VILLAGE (VILLAGE_CODE, VILLAGE_NAME, VILLAGE_ADDRESS, VILLAGE_REGION, VILLAGE_TYPE, VILLAGE_X, VILLAGE_Y, VILLAGE_LNG, VILLAGE_LAT, VILLAGE_BOUNDS, SOURCE) values ('{obj1[i]["poiId"]}','{obj1[i]["poiName"]}','{obj1[i]["poiAddress"]}','{obj1[i]["poiRegion"]}','{obj1[i]["poiType"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiBounds"]}','1');commit;end;";
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
            }
            ...
            for (var i = 0; i < obj1.Count; i++)
            {
                    ...
                    
                {
                    ...
                    DataSet ds = new DataSet();
                        ...
                        {
                            connection.Open();
                            OracleDataAdapter command = new OracleDataAdapter($"begin insert into LBS_BUILDING lb (CODE, BUILDING_NUMBER, BUILDING_ADDRESS, REGION, TYPE, X, Y, LNG, LAT, BOUNDS, VILLAGE_ID, BUILDING_NAME, SOURCE) values ('{obj1[i]["poiId"]}','{obj1[i]["poiBrief"]}','{obj1[i]["poiAddress"]}','{obj1[i]["poiRegion"]}','{obj1[i]["poiType"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiBounds"]}',(select VILLAGE_ID from LBS_VILLAGE lv WHERE lv.VILLAGE_CODE = '{obj1[i]["poiParent"]}' and rownum<2),'{obj1[i]["poiName"]}','1');commit;end;", connection);
                ...
                } else
                {
                    ...
                    {
                        connection.Open();
                        OracleDataAdapter command = new OracleDataAdapter($"begin insert into LBS_BUILDING lb (CODE, BUILDING_NUMBER, BUILDING_ADDRESS, REGION, TYPE, X, Y, LNG, LAT, BOUNDS, BUILDING_NAME, SOURCE) values ('{obj1[i]["poiId"]}','{obj1[i]["poiBrief"]}','{obj1[i]["poiAddress"]}','{obj1[i]["poiRegion"]}','{obj1[i]["poiType"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiLng"]}','{obj1[i]["poiLat"]}','{obj1[i]["poiBounds"]}','{obj1[i]["poiName"]}','1');commit;end;", connection);
